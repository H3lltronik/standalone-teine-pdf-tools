<template>
  <TransitionRoot
    :show="store.progress !== null || store.batchError !== null"
    as="template"
  ...
  >
    <div
      class="fixed left-4 right-4 z-50 top-20 md:left-1/2 md:right-auto md:-translate-x-1/2 md:max-w-xl rounded-xl overflow-hidden shadow-lg"
      :class="store.batchError !== null ? 'bg-red-50 border border-red-200' : 'bg-white border border-slate-200'"
      role="status"
      aria-live="polite"
      ...
      <div class="px-4 py-3 flex items-center gap-3">
        <div class="flex-1 min-w-0">
          <p
            ...
            :class="store.batchError !== null ? 'text-red-800' : 'text-slate-900'"
          >
            {{ statusText }}
          </p>
          <div
            v-if="store.batchError === null"
          ...
          <p
            v-else
            class="mt-2 text-xs text-red-700 line-clamp-2"
          >
            {{ store.batchError }}
          </p>
        </div>
        ...
        >
          <button
            v-if="!store.isCompressing && (store.progress !== null || store.batchError !== null)"
            type="button"
