<template>
  <div class="relative">
    <div v-if="label || minHint" class="flex justify-between items-baseline mb-1.5">
      <label v-if="label" class="text-[10px] font-bold text-slate-700 uppercase tracking-wide">
        {{ label }}
      </label>
      <span v-if="minHint" class="text-[10px] font-medium text-slate-400">{{ minHint }}</span>
    </div>
    <div
